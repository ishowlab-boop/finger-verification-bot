...
FINGERS = {
    "👆": "right index finger raised clearly towards camera",
    "🫆": "index finger prominently visible",
    "✌️": "peace sign with two fingers",
    "👍": "thumb up",
    "👌": "OK hand sign",
    "🤘": "rock on sign",
    "🖐️": "open palm showing all fingers",
}
...
async def edit_image(callback: types.CallbackQuery):
    ...
    try:
        prompt = f"photorealistic edit, the person is clearly holding up their {finger_desc}, same person, same face, same clothes, natural lighting, high detail"
        ...
        await callback.message.answer_photo(
            edited_url,
            caption=f"✅ **AI Finger Edit সফল!**\n\n"
                    f"Name   : {data['username']}\n"
                    f"Finger : {emoji}\n"
                    f"Date   : {datetime.now().strftime('%d/%m/%Y')}"
        )
    except Exception as e:
        await callback.message.answer(f"❌ AI এডিটে সমস্যা:\n{str(e)[:150]}\n\nআবার চেষ্টা করো")
